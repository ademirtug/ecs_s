...
namespace ecs_s {
	...
	void registry::remove_entity(const entity& e) {
		for (auto it : _component_data) {
			it.second->erase(e);
		}
	}
	template<typename T>
	void registry::add_component(const entity& e, T&& p) {
